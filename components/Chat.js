...
const Chat = () => {
  ...
  useEffect(() => {
   ...
   const loadChat = async () => {
     if (token && tripDetails._id) {
      const response = await fetch(`https://grouptravel-backend.onrender.com/chats/chat/${tripDetails._id}`, {
  ...
  const deleteChatMessage = async (tripId, chatId) => {
    try {
      const response = await fetch(`https://grouptravel-backend.onrender.com/chats/chat/${tripId}/${chatId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
