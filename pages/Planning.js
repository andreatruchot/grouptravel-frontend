...
const Planning = () => {
    ...
    useEffect(() => {
        const fetchPlanning = async () => {
            try {
                const response = await fetch(`https://grouptravel-backend.onrender.com/planning/planning/${selectedTripId}`, {
                    headers: {
    ...
    const handleDelete = async (activityId) => {
        if(window.confirm("Êtes-vous sûr de vouloir supprimer cette activité ?")) {
            try {
              const response = await fetch(`https://grouptravel-backend.onrender.com/activities/${activityId}`, {
                method: 'DELETE',
                headers: {
